...
import com.eg.ride.mapper.RideDetailsMapper;
...
public class RideService {
  ...
  public RideResponse requestRide(String authorization, Long userId, RideRequest rideRequest) {
    RideStatusEntity requestedEntity =
      rideStatusRepository.findByValue(RideStatusEnum.REQUESTED.name())
        .orElseThrow(() -> new BadRequestException(STATUS_NOT_FOUND,
          DATA_NOT_FOUND));
    userClient.getUserDetails(authorization, userId);
  ...
  public RideDetailsResponse getRideDetails(String authorization, Long rideId) {
    ...
    return RideDetailsMapper.INSTANCE.map(rideEntity,
