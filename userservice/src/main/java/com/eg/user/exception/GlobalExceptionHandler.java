...
import com.eg.user.exception.model.ErrorMessage;
...
import static com.eg.user.exception.model.ErrorCode.HTTP_MESSAGE_NOT_READABLE;
import static com.eg.user.exception.model.ErrorCode.INCORRECT_PASSWORD_ENTERED;
import static com.eg.user.exception.model.ErrorCode.UNKNOWN_EXCEPTION;
...
public class GlobalExceptionHandler {
  ...
  @ExceptionHandler(IncorrectPasswordEnteredException.class)
  public ResponseEntity<ErrorMessage> handleIncorrectPasswordException(IncorrectPasswordEnteredException exception) {
    log.error("handleIncorrectPasswordException::", exception);
    return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ErrorMessage.builder()
      .message(exception.getMessage())
      .errorCode(INCORRECT_PASSWORD_ENTERED)
      .timestamp(LocalDateTime.now())
      .build());
  }
